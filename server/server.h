...
public:
 ...
 bool start(){
  svr.Get(R"(/(.*))", [&](const Request& req, Response& res) {
    string fileToLook = req.matches[1];
    if(fileToLook.size() > 3){
      if(fileToLook[fileToLook.size() - 3] == 'p' && fileToLook[fileToLook.size() - 2] == 'h' && fileToLook[fileToLook.size() - 1] == 'p') // php file
      {
        string commandLine = "php "; 
        commandLine.append(fileToLook);
        commandLine.append(" > phpResult");
        system(commandLine.c_str());
        std::ifstream t("phpResult");
